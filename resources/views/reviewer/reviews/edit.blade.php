...
<body class="bg-gray-50">
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="mb-8">
            ...
            <p class="mt-2 text-gray-600">Update your review and recommendation</p>
        </div>
        ...
        <div class="bg-white rounded-lg shadow mb-8">
            ...
            <div class="px-6 py-6">
                <div class="mb-6">
                    <h3 class="text-xl font-bold text-gray-900 mb-2">{{ $review->paper_title }}</h3>
                    <p class="text-sm text-gray-600">Conference: {{ $review->conference_name }}</p>
                    @php
                        $deadline = \Carbon\Carbon::parse($review->deadline);
                ...
                @if($review->file_path)
                <div>
                    <a href="{{ route('reviewer.papers.download', $review->assignment_id) }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition">
        ...
        <!-- Review Form -->
        <form action="{{ route('reviewer.reviews.update', $review->review_id) }}" method="POST" class="bg-white rounded-lg shadow">
            @csrf
            @method('PUT')
            ...
            <div class="px-6 py-6 space-y-6">
                ...
                <div>
                    ...
                    <div class="flex items-center space-x-4" x-data="{ score: {{ old('score', $review->score) }} }">
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <label class="flex items-start p-4 border-2 rounded-lg cursor-pointer transition hover:border-blue-500 {{ old('recommendation_code', $review->recommendation_code) === 'ACCEPT' ? 'border-green-500 bg-green-50' : 'border-gray-200' }}">
                            <input type="radio" name="recommendation_code" value="ACCEPT" class="mt-1 mr-3" {{ old('recommendation_code', $review->recommendation_code) === 'ACCEPT' ? 'checked' : '' }}>
                            <div>
                                <div class="font-semibold text-gray-900">Accept</div>
                                <div class="text-sm text-gray-600">Paper is ready for publication as is</div>
                            </div>
                        </label>
                        <label class="flex items-start p-4 border-2 rounded-lg cursor-pointer transition hover:border-blue-500 {{ old('recommendation_code', $review->recommendation_code) === 'MINOR_REVISION' ? 'border-blue-500 bg-blue-50' : 'border-gray-200' }}">
                            <input type="radio" name="recommendation_code" value="MINOR_REVISION" class="mt-1 mr-3" {{ old('recommendation_code', $review->recommendation_code) === 'MINOR_REVISION' ? 'checked' : '' }}>
                            <div>
                        ...
                        <label class="flex items-start p-4 border-2 rounded-lg cursor-pointer transition hover:border-blue-500 {{ old('recommendation_code', $review->recommendation_code) === 'MAJOR_REVISION' ? 'border-yellow-500 bg-yellow-50' : 'border-gray-200' }}">
                            <input type="radio" name="recommendation_code" value="MAJOR_REVISION" class="mt-1 mr-3" {{ old('recommendation_code', $review->recommendation_code) === 'MAJOR_REVISION' ? 'checked' : '' }}>
                            <div>
                                <div class="font-semibold text-gray-900">Major Revision</div>
                                <div class="text-sm text-gray-600">Significant changes required</div>
                            </div>
                        </label>
                        <label class="flex items-start p-4 border-2 rounded-lg cursor-pointer transition hover:border-blue-500 {{ old('recommendation_code', $review->recommendation_code) === 'REJECT' ? 'border-red-500 bg-red-50' : 'border-gray-200' }}">
                            <input type="radio" name="recommendation_code" value="REJECT" class="mt-1 mr-3" {{ old('recommendation_code', $review->recommendation_code) === 'REJECT' ? 'checked' : '' }}>
                            <div>
                ...
                <div>
                    ...
                    >{{ old('comment_author', $review->comment_author) }}</textarea>
                    <div class="mt-2 text-sm text-gray-500" x-data="{ count: {{ strlen(old('comment_author', $review->comment_author ?? '')) }} }">
                ...
                <div>
                    ...
                    >{{ old('comment_chair', $review->comment_chair) }}</textarea>
                    @error('comment_chair')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-between items-center">
                <a href="{{ route('reviewer.reviews.show', $review->review_id) }}" class="px-6 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-100 transition">
                    Cancel
                </a>
