...
class BackupController extends Controller
{
    ...
    public function restore(Request $request, $filename)
    {
        ...
        try {
            ...
            // Wipe the database before restoring to avoid "Table already exists" errors
    ...
    {
        $request->validate([
            'sql_file' => 'required|file|mimetypes:text/plain,application/sql|max:102400', // 100MB max
        ]);

        try {
            $file = $request->file('sql_file');
            $filename = 'uploaded-' . Carbon::now()->format('Y-m-d-H-i-s') . '.sql';
            
            // Store the uploaded file in the backups directory temporarily or permanently
            if (!file_exists(storage_path('app/backups'))) {
                mkdir(storage_path('app/backups'), 0755, true);
            }
            ...
            \Log::info("Starting restore from uploaded file: $filename");
            ...
            return redirect()->route('admin.settings.index')->with('success', 'Phục hồi dữ liệu thành công từ file tải lên.');

        } catch (\Exception $e) {
            \Log::error('Restore upload failed: ' . $e->getMessage());
            return redirect()->route('admin.settings.index')->with('error', 'Lỗi khi phục hồi dữ liệu từ file tải lên: ' . $e->getMessage());
        }
    }
